...
### Changed
- **销量数据真实化**：日均销量控制在 10~50/渠道/SKU，有波动起伏
- **预测达成率调整**：从 100.4% 降至 85.3%（MAPE 17.5%），模拟销售人员乐观偏置

### Added
- **供应计划页公式说明**：ROD 规则 + 采购量公式 + MOQ 约束
- **发货计划页公式说明**：运输方式决策 + 时效参数

### Fixed
- `gen_sample_data.py` 输出路径更新为 `data_input/`，列名匹配新 Schema
...
### Changed
- **日期格式统一为 YYYY-MM**：`Jan-24` → `2024-01`，提升可读性（涉及 demand_forecast.py + Sales_forecasting.csv）

### Added
- **库存计划页公式说明**：可折叠 expander 展示日均需求/安全库存/周转库存/目标库存的计算公式，以及 ABC-XYZ 分类标准和管理策略
...
### Removed
- 删除旧 `sample_data/` 目录（4个CSV），已完全迁移到 `data_input/`
